...
def test_clear_range_requires_expected_name_when_provided() -> None:
    program = _program(primary_name="other_global")
    with pytest.raises(ClearRangeError) as exc:
    ...
    dat = _program(primary_name="DAT_001000")
    assert_clear_range_safe(dat, _Addr(0x1000), _Addr(0x1003), expected_name="g_known")


def test_clear_range_refuses_unrelated_named_start_without_expected_name() -> None:
    program = _program(primary_name="someone_else")
    with pytest.raises(ClearRangeError) as exc:
        assert_clear_range_safe(program, _Addr(0x1000), _Addr(0x1003))
    assert exc.value.payload["error"] == "clear-range-unowned-start"
