...
def test_collect_legacy_only_needs_migration(monkeypatch) -> None:
    from wiz8decomp.callback_typing import collect_callback_typing_plan

    legacy = SimpleNamespace(
        getPathName=lambda: "/wiz8/classes/W8Region",
        getDefinedComponents=lambda: [
            SimpleNamespace(getFieldName=lambda: "region_callback", getDataType=lambda: object())
        ],
    )

    # Force the first class-owned family site to resolve as legacy-only.
    def fake_resolve(program, site):
        if not str(site).startswith("/"):
            return legacy, "/wiz8/classes/W8Region", "legacy-enriched"
    ...
    assert all(row["action"] != "create-and-set-field" for row in legacy_rows)
