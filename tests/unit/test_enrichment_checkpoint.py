...
def test_live_replacement_requires_replace_live_when_project_dir_exists(
    ...
    with pytest.raises(RuntimeError, match="--replace-live"):
        _live_replacement_allowed(
            settings, {"sha256": "seed", "program": "wiz8"}, replace_live=False
        )
    _live_replacement_allowed(settings, {"sha256": "seed", "program": "wiz8"}, replace_live=True)
...
def test_verify_provenance_refuses_missing_current_pdb(
    ...
    with pytest.raises(RuntimeError, match="pdb_sha256 missing on current"):
        _verify_provenance(settings, report, current_seed)
    with pytest.raises(RuntimeError, match="pdb_sha256 missing on current"):
        _verify_provenance(settings, report, current_seed, allow_provenance_mismatch=False)
    warnings = _verify_provenance(settings, report, current_seed, allow_provenance_mismatch=True)
    assert any("pdb_sha256" in item for item in warnings)


def test_verify_provenance_never_bypasses_seed_or_binary_hash(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    settings = _promote_settings(tmp_path)
    monkeypatch.setattr(
    ...
    current_seed = {"sha256": "aa", "program": "wiz8", "binary_sha256": "bb"}
    with pytest.raises(RuntimeError, match="not bypassable"):
        _verify_provenance(settings, report, current_seed, allow_provenance_mismatch=True)
