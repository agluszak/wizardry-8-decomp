...
static unsigned char CheckFileLayout(const OctPreTree* tree, const W8OctPreTreeGeometry* geometry)
{
    ...
    if (LoadWord(bytes, 0x00) != 0x22 || LoadFloat(bytes, 0x02) != spatial->extent_04 ||
        ...
        LoadDword(bytes, 0x56) != tree->m_positional_0a4 ||
        LoadDword(bytes, 0x5a) != tree->m_positional_0a8 ||
        LoadDword(bytes, 0x5e) != tree->m_positional_0ac ||
        LoadWord(bytes, 0x62) != spatial->depth_44 ||
        LoadWord(bytes, 0x64) != spatial->positional_58 ||
        LoadDword(bytes, 0x66) != spatial->positional_74 ||
        LoadDword(bytes, 0x6a) != tree->m_positional_0b4 ||
        LoadDword(bytes, 0x6e) != tree->m_positional_0b8 ||
        ...
        LoadDword(bytes, 0x86) != tree->m_positional_124 || LoadDword(bytes, 0x8a) != 0 ||
        LoadDword(bytes, 0x8e) != 0 || LoadDword(bytes, 0x92) != tree->m_positional_138 ||
        LoadWord(bytes, 0x96) != spatial->region_count_46 ||
        LoadWord(bytes, 0x98) != spatial->leaf_level_52 ||
        LoadDword(bytes, 0x9a) != tree->m_positional_1a8 ||
        LoadDword(bytes, 0x9e) != tree->m_meshCount_1b4 ||
        LoadDword(bytes, 0xa2) != tree->m_positional_1ac ||
        LoadFloat(bytes, 0xa6) != spatial->positional_54 ||
        LoadFloat(bytes, 0xac) != tree->m_region_cell_178 || LoadDword(bytes, 0xb0) != 0 ||
        bytes[0xb8] != 0 || LoadFloat(bytes, 0xb9) != spatial->positional_60 ||
    ...
    cursor += 4;
    cursor += tree->m_positional_0b4 * 0x24;
    cursor += tree->m_positional_0b8 * 0x28;
    cursor += tree->polygon_cursor_3a0 * 4;
    cursor += tree->m_positional_0a4 * tree->m_positional_0a8 * tree->m_positional_0ac * 4;
    cursor += geometry->polygon_count_08 * 4;
    cursor += tree->m_positional_138 * 2;
    cursor += tree->m_positional_124 * 4;
    /* Trigger list skipped (count 0); region array skipped (count <= 1). */
    if (LoadDword(bytes, cursor) != 0xffffffff) {
        ok = 0;
    }
    cursor += 4;
    cursor += (spatial->positional_74 + 1) * 0x10;
...
static unsigned char CheckLoadedSpatial(const OctPreTree* written, const W8Octree* loaded)
{
    ...
    return back->extent_04 == source->extent_04 && back->cell_size_08 == source->cell_size_08 &&
           ...
           back->region_count_46 == static_cast<unsigned short>(written->m_positional_0ac >> 16) &&
           back->leaf_level_52 == source->depth_44 &&
           back->positional_58 == source->positional_58 &&
           loaded->m_positional_0a4 == written->m_positional_0a4 &&
           loaded->m_positional_0a8 == written->m_positional_0a8 &&
           loaded->m_positional_0ac == written->m_positional_0ac &&
           loaded->m_positional_0cc == written->polygon_cursor_3a0 &&
           loaded->m_positional_124 == written->m_positional_124 &&
...
static void RunOctFileRoundTrip(OctFileSemanticResult* result)
{
    ...
    tree->m_owned_0a0[0].positional_00 = 3;
    ...
    tree->m_pSubmeshes[1].positional_0c = 5;
    ...
    if (result->load_ok != 0) {
        ...
        result->leaf_flag_cleared = loaded->m_owned_0a0[0].positional_00 == 2;
        result->tables_roundtrip =
            loaded->m_positional_0b4 == 1 && loaded->m_positional_0b8 == 1 &&
            ...
            loaded->m_pSubmeshes[1].positional_0c == 5 &&
...
bool RunOctFileSemanticTests(OctFileSemanticResult* result)
{
    ...
    return result->octree_io_enabled && result->write_ok && result->file_size_matches &&
           ...
           result->sort_by_key_mixed_ok && result->uv_seam_ok && result->path_node_chunk_ok;
}
...
void PrintOctFileSemanticResults(const OctFileSemanticResult* result)
{
    printf("WIZ8_RUNTIME_TEST scenario=oct-file io_enabled=%u write=%u size=%u "
           ...
           "sortkey_reverse=%u sortkey_mixed=%u uv_seam=%u prepath_chunk=%u\n",
