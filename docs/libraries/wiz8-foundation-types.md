...
The retail link folds identical COMDATs when ICF can prove them equal; the pinned SGP source proves
that with its `DeleteStack`/`DeleteList` and `StackSize`/`ListSize` pairs. Two byte-identical bodies
that remain at different addresses were nevertheless emitted twice from one source. Clustering the
accepted function starts that way — masking relocated operands, `call`/`jmp` displacements and
trailing padding, all of which a second emission changes without the source differing — puts the
growable vector at the top of the first-party results and finds no second class template under it.
What the other large clusters are:
...
`PListInit` allocates ten entries and stores `10` at `+0x04`; `PLAdoptAppend` and `PListInsert` grow
that capacity by five. `PLGet` bounds-checks with a signed `jge`, so the index is `int`; both it and
`PLLength` return zero for a null list rather than faulting. All twelve PList bodies and all ten
IList bodies are now source-owned; the generated comparison report remains the authority for their
...
matching state are generated by `uv run wiz8 report status`.

What separates the two types is the failure value: `IListGetAt` returns `-1` where `PLGet`
...
`ILCreate` allocates a `0x0C`-byte object, which fixes `sizeof(W8IList)` at exactly three ints,
...
the original standalone body as well as its inlined copy inside `ILCreate`.

`ILDestroy` asserts twice under a single null test because `IListFreeData` is inlined into it and
...
original register allocation in that body.

## A folded getter

`0x005E2C70` is the retained body shared by the ordinary `PLLength` and `ILLength` free functions.
It sits **inside** `IList.cpp`'s run, flanked by `IList` functions on both sides, while the other
`PList.cpp` bodies occupy `0x005E22C0`–`0x005E2890`. A pinned VC6 two-unit probe reproduces that
placement: with `IList.cpp` linked first, ICF keeps the count getter in the IList run and redirects
...
inline member names, so the source names them `Set` and `SetFromDouble`; the address markers in
`surrender_math.cpp` keep their identity. The old CFAgent descriptions `VectorFromThreeFloats` and
`Copy3DVector` remain aliases only. Translation-unit reporting classifies both bodies as external
SurRender templates, not recovered first-party Wizardry functions.
