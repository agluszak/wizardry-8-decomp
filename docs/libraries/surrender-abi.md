...
| `srEnvironmentMapper`, `srTriangulator`, exponent tables | No Wizardry path. Environment mapper appears only in provider vtable evidence. | `srEnvironmentMapper` is a vptr-only `srVertexProcessor`; `isActive` returns 1; `process` stays imported (vertex-pipe internals). Global instance `srEnvironmentMapper` at `0x100A48CC`. `srTriangulator::sameSide` / `isInsideTriangle` recovered; list/`next` stay imported. `srExponentTable` is `0x1004` (`float[1024]` plus exponent); `setExponent` fills `pow(k/1023, exponent)`. |
