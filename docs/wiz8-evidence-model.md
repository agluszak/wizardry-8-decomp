...
## The accepted name and its aliases

Ghidra owns the name actually applied to the program. `claimed_name` records why the repository
accepts that name: it is the name supported by the highest-authority origin on the row. When a
lower-authority source calls the same address something else, that spelling stays in the optional
`|`-joined `aliases` column rather than being discarded. The generated Ghidra index validates that
the address still exists; it does not replay the claimed name into the project.
...
```csv
address,claimed_name,owner,confidence,name_origin,authority,aliases
...
`name_origin` and `authority` are required on every reviewed function provenance row: the smaller
targets still use `evidence/reviewed/*/functions.csv`, while Wiz8 uses
`function-provenance.csv`. The closed vocabularies and ceiling rule are validated by
`tests/unit/test_provenance.py`; `wiz8decomp.provenance` owns the vocabulary. Reviewed plate comments
in the canonical Ghidra project link identities back to this provenance rather than reconstructing
the program from the ledger.
...
Current authority counts are generated by `just wiz8 report status` rather than copied into this
document. `evidence/reviewed/wiz8/function-provenance.csv` is the small function entity ledger: it
retains the original address reference, accepted-name provenance, ownership, aliases and source-unit
claim. It deliberately does not store Ghidra's current name or body size, or the matcher's digest.
...
The claim ledger explains accepted facts but cannot recreate Ghidra's functions or types.
