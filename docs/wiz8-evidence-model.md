...
`name_origin` and `authority` are required on every reviewed identity-provenance claim. The smaller
targets still use `evidence/reviewed/*/functions.csv`; Wiz8 source markers and declarations own
recovered identities, while `claims.csv` explains their authority. The closed vocabularies and
ceiling rule are validated by `tests/unit/test_provenance.py`; `wiz8decomp.provenance` owns the
vocabulary.
