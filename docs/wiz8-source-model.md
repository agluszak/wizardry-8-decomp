...
VC6-only. Each component additionally owns its source list and ABI-relevant
compile model once (`src/surrender/sources.cmake`,
`src/srext_jpegimporter/first_party_sources.cmake`,
`src/srext_unzip/first_party_sources.cmake`); both its product target and its
lint object consume those exact variables/functions, including `/Zp4` for
UnZip (layout, accepted by clang-cl) and the `/GX-` exception model for
`plugin.cpp` and SurRender (defined once, applied to the VC6 lane; Clang
rejects `/GX` as unused and never reproduces EH bytes). The product link order
is built from the same first-party list with the vendored IJG/Info-ZIP objects
in their original slot, so a new unit cannot enter one lane without the other.
