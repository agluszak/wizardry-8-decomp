...
def load_local_inputs(settings: Settings) -> dict[str, str]:
    ...
    for role, raw_path in sorted(config.inputs.items()):
        ...
        if relative.is_absolute() or ".." in relative.parts:
            raise ValueError(
                f"local input path must be relative and may not escape WIZ8_INPUT_DIR: {raw_path}"
            )
...
def scan_inputs(settings: Settings) -> InputManifest:
    ...
    for path in sorted(
        (p for p in settings.input_dir.rglob("*") if p.is_file()),
        key=lambda p: p.relative_to(settings.input_dir).as_posix().casefold(),
    ):
        ...
        records.append(
            InputRecord(
                ...
                extension_type_mismatch=extension_mismatch(
                    path, detection.detected_type, detection.container
                ),
    ...
    manifest = InputManifest(
        schema="wiz8.input-manifest",
        files=records,
        ambiguities=[],
    )
    atomic_json(
        settings.build_dir / "manifests" / "input-manifest.json",
