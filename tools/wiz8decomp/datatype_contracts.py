...
def datatype_shape_key(data_type: Any) -> tuple[Any, ...]:
    ...
    if kind == "pointer":
        ...
        return ("pointer", length, datatype_shape_key(pointee))
    if kind == "array":
        ...
        return ("array", length, count, datatype_shape_key(element))
    if kind == "function":
        return ("function", length, function_definition_contract(current))
    if kind == "union":
        ...
        if hasattr(current, "getDefinedComponents"):
            members = tuple(
                (
                    ...
                    datatype_shape_key(component.getDataType()),
                )
    ...
    if kind == "typedef":
        ...
        return ("typedef", length, path, datatype_shape_key(base))
    return (kind, length, path)


def field_descriptors(structure: Any) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "shape": datatype_shape_key(data_type),
                "path": type_identity(data_type),
                "datatype": data_type,
            }
        )
    return rows
...
def structures_field_shape_agree(left: Any, right: Any) -> bool:
    ...
    if (
        ...
        return False
    return fields_shape_agree(field_descriptors(left), field_descriptors(right))
...
def function_definition_contract(definition: Any) -> tuple[Any, ...]:
    """Comparable contract for a FunctionDefinition (return + args + convention)."""

    if definition is None:
        return ()
    ret = (
        datatype_shape_key(definition.getReturnType())
        if hasattr(definition, "getReturnType")
        else ()
    )
    args: list[tuple[str, tuple[Any, ...]]] = []
    if hasattr(definition, "getArguments"):
        for arg in definition.getArguments():
            name = str(arg.getName()) if hasattr(arg, "getName") else ""
            args.append((name, datatype_shape_key(arg.getDataType())))
    convention = ""
    ...
    return (ret, tuple(args), convention)
