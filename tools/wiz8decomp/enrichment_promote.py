...
def _live_replacement_allowed(
    ...
    If a live project exists, require ``--force`` unless freshness proves the
    project is still the untouched reviewed-seed baseline (``current`` or
    ``not-restored``).
    """
    ...
    if not live_exists or status == "not-restored":
        return freshness
    if status == "current":
        # Recorded seed matches reviewed baseline — treat as untouched.
        return freshness
    raise RuntimeError(
...
def _verify_provenance(
    report: dict[str, Any], current_seed: dict[str, Any], *, force: bool
) -> list[str]:
    """Warn/refuse on provenance mismatch when fields are present."""
    ...
    if not isinstance(manifest, dict):
        manifest = {}
    manifest_seed = manifest.get("seed_sha256") if isinstance(manifest, dict) else None
    if manifest_seed and manifest_seed != current_seed["sha256"]:
        msg = (
            f"input-manifest seed_sha256 {manifest_seed} != current reviewed "
            f"{current_seed['sha256']}"
        )
        if force:
            warnings.append(msg)
        else:
...
def run_enrichment_promote(
    ...
    *,
    ...
    provenance_warnings = _verify_provenance(report, current_seed, force=force)
