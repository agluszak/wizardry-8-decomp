...
Trial artifacts live under ``build/enrichment-checkpoint/run-<id>/``
(``ghidra-project/``, ``before.json``, ``after.json``, ``delta.json``,
``pain-*.json``, ``report.json``, ``candidate.gzf``, ``candidate.sha256``,
``input-manifest.json``, per-pass ``steps/``). A convenience
``build/enrichment-checkpoint/latest.json`` may mirror the report after the run.

Prefer ``build/`` over ``work_dir``: Ghidra rejects project path elements that
start with ``.`` (common for work dirs like ``.wiz8-work``).
...
def _snapshot_step_reports(settings: Settings, steps_dir: Path, result: Mapping[str, Any]) -> None:
    """Copy per-pass reports into the run ``steps/`` directory."""

    mapping = (
        ("prototype_repair", "prototype-repair"),
        ("class_structures", "class-structures"),
        ("type_graph", "type-graph-projection"),
        ("legacy_classes_cleanup", "legacy-classes-cleanup"),
        ("vftable_typing", "vftable-typing"),
        ("class_this_typing", "class-this-typing"),
        ("global_typing", "global-typing"),
        ("callback_typing", "callback-typing"),
        ("cosmic_forge_globals", "cosmic-forge-globals"),
        ("function_attributes", "function-attributes"),
        ("surrender_iat", "surrender-iat"),
    )
...
def run_enrichment_checkpoint(
    ...
    *,
    ...
    run_dir = convenience_dir / f"run-{run_id}"
    ...
    iat = run_surrender_iat_typing(
        step_settings,
        program_name=program_name,
        apply=apply_conventions or apply_enrichment,
    )
    steps.append({"step": "surrender-iat", "result": _step_summary(iat)})
    result["surrender_iat"] = _step_summary(iat)
    ...
    _snapshot_step_reports(settings, steps_dir, result)
    ...
    return {
        ...
        "surrender_iat": result.get("surrender_iat"),
        "next": result["next"],
    }
