...
def _untyped_rows(path: Path) -> list[dict[str, str]]:
...
def load_reviewed_class_model(repo_dir: Path, program: str) -> ReviewedClassModel:
    ...
    classes = tuple(
        ReviewedClass(
            ...
            destructor=parse_hex(
                row.get("destructor", ""),
                field="destructor",
                path=classes_path,
                allow_empty=True,
            ),
            scalar_deleting_destructor=parse_hex(
        ...
        for row in read_table(classes_path, program=program).rows
    ...
    vtables = tuple(
        ReviewedVtable(
            ...
            class_name=row["class_name"].strip(),
            address=parse_hex(row["address"], field="address", path=vtables_path) or 0,
            subobject_offset=parse_hex(
                row["subobject_offset"],
                field="subobject_offset",
                path=vtables_path,
                allow_empty=True,
            ),
            kind=row["kind"].strip(),
        ...
        for row in read_table(vtables_path, program=program).rows
        if _accepted(row, program)
    )
    ...
    slots = tuple(
        ReviewedVtableSlot(
            ...
            target=parse_hex(row["target"], field="target", path=slots_path) or 0,
            name=row["slot_name"].strip(),
            evidence_id=row["evidence_id"].strip(),
        )
        for row in read_table(slots_path, program=program).rows
        if _accepted(row, program)
    )
