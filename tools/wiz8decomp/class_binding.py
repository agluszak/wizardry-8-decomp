...
def _is_ghidra_class(namespace: Any) -> bool:
    """True when ``namespace`` is a real ``GhidraClass`` (not a plain namespace)."""

    if namespace is None:
        return False
    ghidra_class_type: type | None = None
    try:
        from ghidra.program.model.symbol import GhidraClass  # type: ignore[import-not-found]

        ghidra_class_type = GhidraClass
    except Exception:  # noqa: BLE001 — unit fakes / stubs
...
def find_ghidra_class(program: Any, owning_class: str) -> Any | None:
    ...
    if existing is None or not _is_ghidra_class(existing):
        return None
    return existing
...
def find_class_structure(program: Any, ghidra_class: Any) -> Any | None:
    ...
    if ghidra_class is None or not _is_ghidra_class(ghidra_class):
        return None
