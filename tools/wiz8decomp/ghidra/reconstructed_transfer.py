        ...
        with pyghidra.program_context(project, "/" + program_name) as program:
            ...
            try:
                ...
                for transfer in plan.transfers:
                    ...
                    # The whole signature applies or none of it does. A type the
                    # program does not know makes the parser refuse, which is
                    # the wanted answer: a list where one parameter is real and
                    # the next is `undefined4` cannot be told apart later from
                    # a fully recovered one.
                    signature = transfer.signature
                    definition = None
                    try:
                        definition = CParserUtils.parseSignature(
                            None, program, signature.spelling(function.getName())
                        )
                    except Exception:  # noqa: BLE001 - the parser reports by throwing
                        definition = None
                    ...
                    for name, frame_offset, _type in transfer.frame_variables:
...
def transfer_into_overlay(
    settings: Settings, selector: str, hypothesis: str, build_dir: Path | None = None
) -> dict[str, Any]:
