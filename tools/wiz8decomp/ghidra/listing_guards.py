...
def _is_owned_or_debris_symbol(name: str, owner: str | None) -> bool:
    ...
    if name == owner:
        return True
    return name.startswith((f"{owner}[", f"{owner}."))


def _is_default_or_unnamed_symbol(name: str | None) -> bool:
...
def _start_primary_allowed(start_primary: str | None, expected_name: str | None) -> bool:
    """True when the listing primary at ``start`` may be replaced.

    Accept the expected source symbol, ``DAT_…`` defaults, unnamed/default
    labels, or no symbol. An unrelated named global is a conflict.
    """

    if _is_default_or_unnamed_symbol(start_primary):
        return True
    if expected_name is None:
        return False
    if start_primary == expected_name:
        return True
    return _is_owned_or_debris_symbol(start_primary, expected_name)
...
def assert_clear_range_safe(
    ...
    *,
    ...
    if not address_owned and not _start_primary_allowed(start_primary, expected_name):
        raise ClearRangeError(
            {
                "error": "clear-range-name-mismatch"
                if expected_name is not None
                else "clear-range-unowned-start",
