...
def decide_field_action(
    ...
    *,
    ...
    if int(bound.getLength()) != int(evidence.getLength()):
        return "conflict"
    if not structures_field_shape_agree(bound, evidence):
        return "conflict"
    legacy = has_legacy_nested_ref(bound) if nested_legacy is None else nested_legacy
...
def _discover_nested_structure_names(evidence: Any | None, known: set[str]) -> set[str]:
    ...
    for nested in walk_datatype_refs(evidence):
        ...
        name = nested.getName() if hasattr(nested, "getName") else None
        if not name:
            continue
        simple = str(name)
        if simple in known or simple in discovered:
            continue
        if is_legacy_path(path) or path.startswith("/"):
            discovered.add(simple)
    return discovered
...
def _attach_field_actions(
    ...
    for owning, row in identity.items():
        ...
        action = decide_field_action(bound, evidence)
...
def _remap_datatype(program: Any, data_type: Any, identity: Mapping[str, Mapping[str, Any]]) -> Any:
    ...
    # Preserve settings-bearing typedefs: remap their base only when plain.
    ...
    if isinstance(current, Union):
        # reccmp cannot write unions; refuse mutation when members need remap.
        if has_legacy_nested_ref(current):
            return current  # caller treats unresolved union remaps as conflict
        return current
    ...
    if isinstance(current, Structure):
        ...
        for key, row in identity.items():
            if _simple_name(key) != name and key != name:
                continue
            bound_path = row.get("bound_path")
            if not bound_path or is_legacy_path(bound_path):
                continue
            if path == bound_path:
                return current
            if is_legacy_path(path) or path == row.get("evidence_path"):
                bound = program.getDataTypeManager().getDataType(bound_path)
                if bound is not None:
                    return bound
        return current

    # Legacy path by identity string.
    path = type_identity(current)
    if is_legacy_path(path):
        leaf = path.rsplit("/", 1)[-1]
        for key, row in identity.items():
            if _simple_name(key) == leaf or key == leaf:
                bound_path = row.get("bound_path")
