...
extract_app = typer.Typer(help="Extract configured inputs without modifying them.", no_args_is_help=True)
variants_app = typer.Typer(help="Materialize and compare independent build variants.", no_args_is_help=True)
ghidra_app = typer.Typer(help="Own Ghidra projects, queries, daemon, and exports.", no_args_is_help=True)
...
def _tracked_copyrighted(settings: Any) -> list[str]:
    if not (settings.repo_dir / ".git").exists():
        return []
    completed = subprocess.run(["git", "ls-files", "-z"], cwd=settings.repo_dir, capture_output=True, check=False)
    suspect_suffixes = {".exe", ".dll", ".iso", ".zip", ".7z", ".rar", ".cab", ".bik", ".wav", ".mp3", ".slf", ".asi", ".m3d"}
    suspects = []
    for raw in completed.stdout.split(b"\0"):
        ...
        if path.suffix.casefold() in suspect_suffixes and not relative.startswith("vendor/ghidra/exports/"):
...
def doctor() -> None:
    ...
    def action() -> dict[str, Any]:
        ...
        checks.append({"name": "ghidra", "ok": version == REQUIRED_GHIDRA_VERSION and release == REQUIRED_GHIDRA_RELEASE, "expected": f"{REQUIRED_GHIDRA_VERSION} {REQUIRED_GHIDRA_RELEASE}", "actual": f"{version} {release}", "path": str(settings.ghidra_install_dir)})
        try:
            pyghidra_version = importlib.metadata.version("pyghidra")
        except importlib.metadata.PackageNotFoundError:
            pyghidra_version = None
        checks.append({"name": "pyghidra", "ok": pyghidra_version == REQUIRED_PYGHIDRA_VERSION, "expected": REQUIRED_PYGHIDRA_VERSION, "actual": pyghidra_version})
        checks.append({"name": "input-directory", "ok": settings.input_dir.is_dir() and os.access(settings.input_dir, os.R_OK), "path": str(settings.input_dir)})
...
def ghidra_import(
    all_programs: bool = typer.Option(False, "--all", help="Import every configured analysis target."),
    ...
    _run_action(lambda: import_programs(_settings(), all_modules=all_programs, variant=variant, requested_program=program))
...
@ghidra_app.command("query", context_settings={"allow_extra_args": True, "ignore_unknown_options": True})
...
def ghidra_export_evidence(
    ...
    _run_action(lambda: export_evidence(_settings(), selector=program, address=address, export_all=all_functions))
