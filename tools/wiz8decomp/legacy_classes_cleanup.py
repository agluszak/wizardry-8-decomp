...
def _legacy_category_datatypes(program: Any) -> list[Any]:
    """Walk ``/wiz8/classes`` recursively, then union any leftover ``is_legacy_path`` types."""

    manager = program.getDataTypeManager()
    found: list[Any] = []
    seen: set[str] = set()

    def _walk(cat: Any) -> None:
        if cat is None:
            return
        ...
        get_cats = getattr(cat, "getCategories", None)
...
def _qualified_from_legacy_path(path: str) -> str:
    rest = path.removeprefix("/wiz8/classes/")
    while rest.endswith("*"):
        rest = rest[:-1].rstrip()
    ...
    *,
    ...
    if identity_map_or_bindings:
        ...
        if qualified and qualified in identity_map_or_bindings:
            ...
            keys = [
                key
                for key in identity_map_or_bindings
                if key == leaf or key == qualified or _simple_name(key) == leaf
            ]
            if qualified:
                exact = [key for key in keys if key == qualified]
                if exact:
                    keys = exact
            if len(keys) > 1:
                return None, None, "ambiguous-leaf"
        for key in keys:
            row = identity_map_or_bindings[key]
...
def classify_legacy_datatype(
    ...
    *,
    ...
    bound, bound_path, how = _bound_for_legacy(
        program,
        leaf,
        identity_map_or_bindings,
        qualified=_qualified_from_legacy_path(path),
    )
    ...
    *moves* the bound Structure into ``/wiz8/classes`` (same name, new path),
    ...
    def _apply_one(_program: Any, row: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if _is_pointer_wrapper_name(str(row.get("name") or "")):
            ...
            pointee = bound
            for _ in range(max(str(row.get("name") or "").count("*"), 1)):
