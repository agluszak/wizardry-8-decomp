...
unsigned char Function5058A0(W8MonsterInfo* source, W8MonsterInfo* target,
                             W8VisibilityRecord* record); /* 0x005058A0 */
float Function505A40(float subject_x, float subject_y, float subject_z, float target_x,
                     float target_y, float target_z, float yaw, unsigned int attribute,
                     int missile_bonus, unsigned char blind, unsigned char spell_kind, int arg_12,
                     int arg_13, int arg_14, int arg_15, float distance); /* 0x00505A40 */
