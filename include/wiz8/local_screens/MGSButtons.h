...
/* Which menu the open sub-menu panel serves and each row's entry/state. */
...
/* Store the (menu, item) pair's pending command in the level block. */
void MapSubMenuSelection(short menu, short item); /* 0x00596240 */
/* Whether the slot may perform the pending command; 0 no, 1 pending, 2/3
   gated. */
int CheckSubMenuActionUsable(int party_slot); /* 0x00596360 */
/* The row availability states the refresh maps icon frames through. */
short GetSubMenuEntryState(short menu, short item, int party_slot); /* 0x00595FE0 */
