...
/* Trigger's m_pActionData points at this polymorphic payload. Type-5 and
   ...
   Retail installs final vtable 0x005EC148 for the type-5 allocation in
   Trigger::Run and returns to the base table at 0x005EC138 during destruction;
   that distinct final table plus its scalar deleting destructor are ABI
   artifacts without added storage or non-lifecycle behavior, so they do not
   justify a separate authored subclass. */
