...
{
    MSYS_SetBtnUserData(button, 0, reinterpret_cast<INT32>(data)); // reinterpret-ok: SGP userdata slot carries the pointer
}

template <typename T>
inline T* GetButtonUserDataPointer(GUI_BUTTON* button)
{
    return reinterpret_cast<T*>(MSYS_GetBtnUserData(button, 0)); // reinterpret-ok: SGP userdata slot carries the pointer
}
