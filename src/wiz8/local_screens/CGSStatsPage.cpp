...
W8CharacterStatsValue005EF6B0::W8CharacterStatsValue005EF6B0(
    ...
    : W8TextControl(owner, 0xffffffff, x, y, 0, 0, 0x104, 0,
                    1, 1, 2, 2, 3)
{
    m_default_record_0bc = default_record;
    m_textBuffer.SetText(gppStringList[default_record->name_id_0c],
                         g_options_detail_font_683614);
    m_textBuffer.SetLayoutMode(g_W8TextBufferLayoutMask005ED554 |
...
void W8CharacterStatsValue005EF6B0::SetRecord(
...
W8CharacterStatsRecordControl005EF700::W8CharacterStatsRecordControl005EF700(
    Controls* owner, int top, int height,
    const W8CharacterStatsRecord* record, int variant)
    : W8TextControl(owner, 0xffffffff, 0, top, 0x7e, top + height,
                    0x104, 0, 0, 0, 0, 0, 0)
{
    m_record_0b8 = record;
    ...
    m_textBuffer.SetLayoutMode(g_W8TextBufferLayoutMask005ED554 |
                               g_W8TextBufferLayoutMask005ED548);
    m_pressedTextOffset = 0;
    UpdateTextBounds(0x19, m_text_offset_0c0 + top, 0x7e,
                     m_text_offset_0c0 + top + 0x16);
}
...
void W8CharacterStatsRow005EF750::Initialize(
    Controls* owner, unsigned int* region_set, int x, int y, int count,
    ...
    int help_second, int help_value)
{
    ...
    m_decrement_01c = new W8TextControl(
        owner, 0xffffffff, x + 1, y + 4, 0, 0, 0x10a, 0,
        0, 2, 1, 4, 3);
    m_decrement_01c->m_listener = this;
    m_decrement_01c->EnableRegionHelp(help_first);

    m_increment_020 = new W8TextControl(
        owner, 0xffffffff, x + 0x9f, y + 4, 0, 0, 0x10a, 0,
        5, 7, 6, 9, 8);
    m_increment_020->m_listener = this;
    m_increment_020->EnableRegionHelp(help_value);

    m_value_control_024 =
        new W8CharacterStatsValue005EF6B0(owner, x + 0x1c, y + 4,
                                          default_record);
...
{
    if (m_subpanel_028 == 0) {
        m_subpanel_028 = new Controls(
            m_x_00c + 0x9e, m_y_010 + 1, m_x_00c + 0x11c,
            m_y_010 + 5 + (unsigned int)m_count_008 * 0x16, -1, 0, -1);
        m_subpanel_028->AcquireRegionSet(m_region_set_014);
        m_subpanel_entries_02c =
            new W8TextControl*[m_count_008];
        ...
        for (unsigned int index = 0; index < m_count_008; ++index) {
            ...
            m_subpanel_entries_02c[index] =
                new W8CharacterStatsRecordControl005EF700(
    ...
    for (unsigned int index = 0; index < m_count_008; ++index) {
        m_subpanel_entries_02c[index]->SetEnabled(
            m_table_018[index].enabled_0e);
    }
}
...
void W8CharacterStatsRow005EF750::OnPrimary(W8TextControl* control)
{
    if (control == m_decrement_01c) {
        ...
        if (index >= 0) {
            ...
            do {
                if (record->enabled_0e != 0) {
                    m_value_control_024->SetRecord(
                        index == -1 ? 0 : &m_table_018[index]);
        ...
        if ((m_value_control_024->m_stateFlags &
...
W8CharacterStatsRow005EF750::W8CharacterStatsRow005EF750()
    : m_value_004(-1), m_count_008(0), m_table_018(0), m_decrement_01c(0),
      m_increment_020(0), m_value_control_024(0), m_subpanel_028(0),
      m_subpanel_entries_02c(0), m_listener_030(0)
{
}
...
void W8CharacterPage005EF778::UpdateRowValues()
{
    ...
    if (m_mode_068 == 0) {
        ...
        DetermineEligibleProfessions(m_character_060, m_creation_state_064,
                                     eligible);
    }
    for (int index = 0; index < 15; ++index) {
        if (m_mode_068 == 0) {
            ...
            g_character_profession_records_0064f028[index].enabled_0e =
                eligible[index];
        }
    }
...
void W8CharacterPage005EF778::OnRowInfoRequested(
    W8CharacterStatsRow005EF750* row, int value)
{
    if (row == m_profession_row_07c) {
    ...
    W8Character* character, W8CharacterCreationState* creation_state, int mode)
{
    ...
    m_profession_row_07c->Initialize(
        ...
        0xf, g_character_profession_records_0064f028,
        &g_character_profession_default_record_0064f248, 0xf7, 0xf6, 0xf8);
    m_race_row_080->Initialize(
    ...
    for (int attribute_index = 0; attribute_index < 7; ++attribute_index) {
        W8CharacterPageEntry* entry = new W8CharacterPageEntry(
            this, 0xe5, 0x1b + attribute_index * 0xe, 0);
        AddEntry(entry);
        entry->m_listener_004 = this;
        entry->SetContent(
            attribute_index,
            gppStringList[g_character_description_first_ids_61e3a4
                              [attribute_index]],
    ...
    for (int control_index = 0; control_index < 5; ++control_index) {
        W8TextControl* control = new W8TextControl(
            this, 0xffffffff, 0xf3, 0xba + control_index * 0xe, 0x195,
...
void W8CharacterPage005EF778::Redraw()
{
    ...
    if (redraw) {
        ...
        text.SetLayoutMode(g_W8TextBufferLayoutMask005ED554 |
        ...
        text.SetLayoutMode(g_W8TextBufferLayoutMask005ED548 |
        ...
        if (m_character_060->current_profession != -1 ||
                ...
                text.SetText(FormatWideString(
                    ...
                    gppStringList[g_character_skill_name_ids_61e454
                                      [g_profession_bonus_skills
                                           [m_character_060->current_profession]]],
                    gppStringList[0xb2]), g_font_683660);
                text.RenderToTarget(0, 0, -14);
            ...
            for (int trait_index = 0; trait_index < 0x20; ++trait_index) {
                if (available[trait_index] != 0) {
                    text.SetLayoutBounds(&bounds, 1, 1);
                    text.SetText(gppStringList[
                                     g_character_trait_name_ids_61e530[trait_index]],
                                 g_font_683660);
                    text.RenderToTarget(0, 0, -14);
                    bounds.top = bounds.top + line_height;
                }
        ...
        text.RenderToTarget(0, 0, -14);
        if (m_character_060->race != -1) {
            text.SetLayoutMode(g_W8TextBufferLayoutMask005ED550 |
                               g_W8TextBufferLayoutMask005ED554);
            text.SetText(FormatWideString(
                g_format_d_0060aa20, m_character_060->hp_max), g_font_683660);
            text.RenderToTarget(0, 0, -14);
        }
        ...
        if (m_character_060->race != -1) {
            text.SetLayoutMode(g_W8TextBufferLayoutMask005ED550 |
                               g_W8TextBufferLayoutMask005ED554);
            text.SetText(FormatWideString(
                g_format_d_0060aa20, m_character_060->stamina_max), g_font_683660);
            text.RenderToTarget(0, 0, -14);
        }
        ...
        if (m_character_060->race != -1) {
            text.SetLayoutMode(g_W8TextBufferLayoutMask005ED550 |
                               g_W8TextBufferLayoutMask005ED554);
            text.SetText(FormatWideString(
                g_format_d_0060aa20, m_character_060->armor_class_average),
                g_font_683660);
            text.RenderToTarget(0, 0, -14);
        }
        ...
        if (m_character_060->race != -1) {
            text.SetLayoutMode(g_W8TextBufferLayoutMask005ED550 |
                               g_W8TextBufferLayoutMask005ED554);
            text.SetText(FormatWideString(
                g_format_d_0060aa20, m_character_060->carrying_capacity / 10),
                g_font_683660);
            text.RenderToTarget(0, 0, -14);
        }
        ...
        if (m_character_060->race != -1 ||
            ...
            for (int realm = 0; realm < 6; ++realm) {
                ...
                if (value == 0) {
                    text.SetText(FormatWideString(g_dash_0064789c),
                                 g_font_683660);
                }
                else {
                    text.SetText(FormatWideString(g_format_plus_d_0064dc24, value),
                                 g_font_683660);
                }
                text.RenderToTarget(0, 0, -14);
            }
        ...
        if (m_character_060->current_profession != -1) {
            ...
            text.SetLayoutBounds(&bounds, 1, 1);
            text.SetText(gppStringList[g_character_skill_name_ids_61e454[
                             g_profession_bonus_skills
                                 [m_character_060->current_profession]]],
                         g_font_683660);
            text.RenderToTarget(0, 0, -14);
            for (int index = 0; index < 4; ++index) {
                if (g_profession_skills
                        [m_character_060->current_profession][index] != -1) {
                    bounds.top = bounds.top + 0xe;
                    bounds.bottom = bounds.bottom + 0xe;
                    text.SetLayoutBounds(&bounds, 1, 1);
                    text.SetText(gppStringList[g_character_skill_name_ids_61e454[
    ...
    if (m_dirty_06d) {
        ...
        text.SetLayoutBounds(&bounds, 1, 1);
        text.SetText(FormatWideString(g_format_d_0060aa20,
                                      m_creation_state_064->attribute_step_limit),
                     g_options_detail_font_683614);
        text.RenderToTarget(0, 1, -14);
        bounds.top = 0x184;
        bounds.bottom = 0x19b;
