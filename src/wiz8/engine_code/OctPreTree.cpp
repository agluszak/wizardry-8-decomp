                                               ...
                                               W8GameData* game_data)
{
    ...
    if (m_positional_0b8 != 0) {
        for (unsigned long index = 0; index < m_positional_0b8; ++index) {
            m_owned_0a0[index].positional_00 &= 0xfffffffe;
        }
    }
    ...
    for (int axis = 0; axis < 3; ++axis) {
        ...
        header.grid_dims_56[axis] = (&m_positional_0a4)[axis];
    }
    header.depth_62 = spatial_000.depth_44;
    header.positional_64 = spatial_000.positional_58;
    header.positional_9a = m_positional_1a8;
    header.polygon_count_66 = spatial_000.positional_74;
    header.branch_count_6a = m_positional_0b4;
    header.region_count_96 = spatial_000.region_count_46;
    header.positional_98 = spatial_000.leaf_level_52;
    header.mesh_total_9e = m_meshCount_1b4;
    header.leaf_count_6e = m_positional_0b8;
    ...
    header.positional_86 = m_positional_124;
    header.polygon_cursor_82 = polygon_cursor_3a0;
    header.triangle_count_8a = m_positional_128;
    header.positional_92 = m_positional_138;
    header.region_cell_ac = m_region_cell_178;
    header.positional_b9 = spatial_000.positional_60;
    header.particle_len_c5 = m_usMeshParticlesLen_0e8;
    header.positional_a6 = spatial_000.positional_54;
    ...
    header.positional_a2 = m_positional_1ac;
    header.zero_8e = 0;
    if (pre_pathing_2a0 == 0) {
        ...
        header.pathing_b0 = pre_pathing_2a0->edge_node_count_008;
    }
    ...
    if (FileWrite(file, m_owned_0d0, header.polygon_cursor_82 * 4, 0) == 0) {
        Function497690(7, "WriteOctFile: Couldn't write Poly List info.\n");
        return 0;
    }
    unsigned int grid_cells = m_positional_0ac * m_positional_0a8 * m_positional_0a4;
    ...
    if (header.positional_92 != 0 &&
        FileWrite(file, m_owned_148, header.positional_92 * 2, 0) == 0) {
        Function497690(7, "WriteOctFile: Couldn't write region list.\n");
        return 0;
    }
    if (header.positional_86 != 0 &&
        FileWrite(file, m_owned_12c, header.positional_86 * 4, 0) == 0) {
    ...
    if (header.triangle_count_8a != 0 &&
        FileWrite(file, m_owned_130, header.triangle_count_8a * 4, 0) == 0) {
    ...
    FileWrite(file, &sentinel, 4, 0);
    if (header.polygon_count_66 != 0) {
        if (FileWrite(file, m_pSubmeshes, (header.polygon_count_66 + 1) * 0x10, 0) == 0) {
        ...
        if (m_ulNumParticles != 0) {
            if (FileWrite(file, m_pusMeshParticleLookup, header.positional_a2 * 2 + 2, 0) == 0) {
        ...
        if (m_ulNumProps != 0) {
            if (FileWrite(file, m_pusMeshPropLookup, header.positional_a2 * 2 + 2, 0) == 0) {
    ...
    if (game_data != 0 && header.positional_86 != 0) {
...
OctMeshModel* OctPreTree::CreateSubMeshes00468C30(W8OctPreTreeGeometry* geometry)
{
    ...
    if (spatial_000.positional_74 == 0) {
        return 0;
    }
    m_vertex_count_0c8 = geometry->vertex_count_00;
    VerifyPolygonRegions0046ABF0();
    records = static_cast<W8OctSubmeshBuild*>(malloc((spatial_000.positional_74 + 1) * 0x9c));
    if (records == 0) {
        Function497690(7, "\nCreateSubMeshes: Could not allocate submeshes.\n");
    } else {
        memset(records, 0, (spatial_000.positional_74 + 1) * 0x9c);
        ...
        if (m_owned_0d4 == 0) {
            ...
            FreeSubmeshBuildArrays(records, spatial_000.positional_74);
        } else {
            m_pSubmeshes =
                static_cast<W8OctSubmesh*>(malloc((spatial_000.positional_74 + 1) * 0x10));
            models = new OctMeshModel[spatial_000.positional_74 + 1];
            if (m_pSubmeshes != 0 && models != 0) {
                memset(m_pSubmeshes, 0, (spatial_000.positional_74 + 1) * 0x10);
                root_count = 0;
                kind_counts[3] = 0;
                kind_counts[2] = 0;
                kind_counts[1] = 0;
                ...
                record_index = 1;
                model_index = 0;
                if (spatial_000.positional_74 > 1) {
                    ...
                    do {
                        ...
                        submesh->positional_0c = record->polygon_count_1c;
                        submesh->positional_08 = record->next_link_10;
                        ...
                        model->uv_count_08 = record->next_link_10 - 1;
                        ...
                        ++model_index;
                        ++record_index;
                    } while (record_index < spatial_000.positional_74);
                }
                spatial_000.positional_74 = model_index;
                m_meshCount_1b4 = root_count;
                m_positional_1a8 = kind_counts[0];
                m_positional_1ac = kind_counts[1];
                ...
                /* Verified retail behavior: positional_74 was just overwritten
...
unsigned long OctPreTree::SplitMeshes00469670(W8OctPreTreeGeometry* geometry,
                                              W8OctSubmeshBuild* records)
{
    unsigned long count = spatial_000.positional_74;
    if (count == 0) {
        return 0;
    }
    ...
    m_vertex_count_0c8 = geometry->vertex_count_00;
    count = spatial_000.positional_74;
    m_meshCount_1b4 = count;
    m_positional_1a8 = count;
    ...
    if (spatial_000.positional_74 == m_positional_1a8) {
        spatial_000.positional_74 = next_free;
    } else {
        spatial_000.positional_74 = next_free;
    ...
    if (spatial_000.positional_74 > 1) {
        for (unsigned long index = 1; index < spatial_000.positional_74; ++index) {
            W8OctSubmeshBuild* record = records + index;
    ...
    m_positional_1a8 = kind_counts[0];
    m_positional_1ac = kind_counts[1] + kind_counts[0];
    free(staging);
    free(vertex_ids);
    free(keys);
    free(scratch);
...
struct W8OctUvPoolEntry0046A4B0 {
    long link;
    float u;
    float v;
};
...
{
    W8OctUvPoolEntry0046A4B0* table =
        static_cast<W8OctUvPoolEntry0046A4B0*>(malloc(record->polygon_count_1c * 0x30));
    if (table == 0) {
    ...
    for (unsigned long poly = 0; poly < record->polygon_count_1c; ++poly) {
        ...
        for (unsigned long corner = 0; corner < 3; ++corner) {
            ...
            W8OctUvPoolEntry0046A4B0* entry = &table[last];
            if (table[last].link < 0) {
...
unsigned long OctPreTree::AllocateSubMesh0046A790(W8OctSubmeshBuild* records)
{
    if (spatial_000.polygon_count_3c > 1) {
        for (unsigned long poly = 1; poly < spatial_000.polygon_count_3c; ++poly) {
            ...
            if (region == 0 || region >= spatial_000.positional_74) {
                char text[1024];
    ...
    for (unsigned long index = 1; index < spatial_000.positional_74; ++index) {
        ...
        while (slot != -1) {
            if (cells->entries[slot].key == static_cast<short>(index)) {
                ...
                float cell_x =
                    static_cast<float>((cell >> 0x10) & 0xff) * spatial_000.positional_54 +
                    spatial_000.minimum_0c.x;
                float cell_y = static_cast<float>((cell >> 8) & 0xff) * spatial_000.positional_54 +
                               spatial_000.minimum_0c.y;
                float cell_z = static_cast<float>(cell & 0xff) * spatial_000.positional_54 +
                               spatial_000.minimum_0c.z;
                if (cell_x + spatial_000.positional_54 < min_x || max_x < cell_x ||
                    cell_y + spatial_000.positional_54 < min_y || max_y < cell_y ||
                    cell_z + spatial_000.positional_54 < min_z || max_z < cell_z) {
    ...
    for (unsigned long poly = 1; poly < game_data_3a4->polygon_count_08; ++poly) {
        ...
        if (polygon->region_32 >= spatial_000.region_count_46) {
            ...
            cell[1] = static_cast<unsigned int>(
                (polygon->position_18.x - spatial_000.minimum_0c.x) / spatial_000.positional_54);
            cell[2] = static_cast<unsigned int>(
                (polygon->position_18.y - spatial_000.minimum_0c.y) / spatial_000.positional_54);
            cell[3] = static_cast<unsigned int>(
                (polygon->position_18.z - spatial_000.minimum_0c.z) / spatial_000.positional_54);
    ...
    for (unsigned long mesh = 1; mesh < m_meshCount_1b4; ++mesh) {
        ...
        while (slot != -1) {
            if (automesh_cells_29c->entries[slot].key == static_cast<short>(mesh)) {
                ...
                cell[3] = packed & 0xff;
                float cell_x = static_cast<float>(cell[1]) * spatial_000.positional_54 +
                               spatial_000.minimum_0c.x;
                float cell_y = static_cast<float>(cell[2]) * spatial_000.positional_54 +
                               spatial_000.minimum_0c.y;
                float cell_z = static_cast<float>(cell[3]) * spatial_000.positional_54 +
                               spatial_000.minimum_0c.z;
                int node = DescendByMask(cell);
                if (node != 0) {
                                                 ...
                                                 unsigned int min_component_percent)
{
    ...
    W8HashTable<unsigned int, unsigned int*> cond_map;
    char message[0x400];
                ...
                while (SnapToGround(&node, 1)) {
                    ...
                    if (current_sector >= 0) {
                        ...
                        if (!SnapToGround(&node, 0) || g_float_005ec3f8 < fabsf(node.y - snapped)) {
                            supports_2b0[m_lNumSupports_2a8] = current_sector;
                            ++m_lNumSupports_2a8;
                        }
                        node.y = snapped;
                    }
    ...
    if (path_node_count_2a4 != 0) {
        ...
        pre_pathing_2a0->ConfigureForLevel(path_node_count_2a4, m_region_cell_178,
                                           static_cast<int>(m_positional_17c),
...
char OctPreTree::PathNodeObstructed0046B700(const srVector3T<float>* node)
{
    ...
    float clearance;
    memcpy(&clearance, &m_positional_17c, sizeof(clearance));
                ...
                if (current_sector >= 0) {
                    ...
                    if (probe == 0 || clearance * static_cast<float>(g_double_005ebe80) <
                        ...
                        for (int b = 0; b < m_lNumBlocks_2ac && result != 1; ++b) {
                            if (current_sector == blocks_328[b])
                                result = 1;
                        }
                        out = result;
                        if (result != 1) {
                            ...
                            for (int s = 0; s < m_lNumSupports_2a8; ++s) {
                                if (current_sector == supports_2b0[s])
                                    absent = false;
                            }
                            if (absent) {
                                supports_2b0[m_lNumSupports_2a8] = current_sector;
    ...
    /* Verified retail order: the appends above write supports_2b0[30]/
       blocks_328[30] before this check ever runs, so a 31st entry goes out
...
/* Cross-links each support/blocker sector to the prop that produced it and
   stores a {node, cell} pair in the conditional-node map keyed by
   (prop_number<<16 | prop_index+1). Blocker nodes are flagged 0x2000000. */
// FUNCTION: WIZ8 0x0046b9d0
unsigned char
OctPreTree::InsertConditionalNodes0046B9D0(W8HashTable<unsigned int, unsigned int*>* nodes,
                                           ...
                                           W8PreProp* preprops, int preprop_count)
{
    ...
    for (int s = 0; s < m_lNumSupports_2a8; ++s) {
        int sector = supports_2b0[s];
        bool found = false;
        for (int p = 0; p < preprop_count && !found; ++p) {
            W8PreProp* pp = preprops + p;
            if (pp->num_stop_meshes_40 != 0 &&
                static_cast<int>(pp->first_prop_number_42) <= sector &&
                sector < static_cast<int>(pp->first_prop_number_42 + pp->num_stop_meshes_40)) {
                unsigned int key =
                    (static_cast<unsigned int>(
                         pp->pStopMeshes[sector - pp->first_prop_number_42].m_prop_number_02)
                     << 16) |
                    static_cast<unsigned int>(p + 1);
                unsigned int* cond_node = static_cast<unsigned int*>(malloc(8));
                if (cond_node == 0) {
                    srAssertFail(
    ...
    for (int k = 0; k < count; ++k) {
        ...
        if (prop->BoundsOverlap004B7620(minimum, maximum) != 0) {
            ...
            if ((prop->m_flags_00 & 1) != 0)
                return 1;
            if (m_lNumSupports_2a8 != 0 && current_sector == supports_2b0[0])
                return 1;
            blocks_328[m_lNumBlocks_2ac] = id;
            ++m_lNumBlocks_2ac;
        }
    }
    if (prop_hit)
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (prop->num_frame_pos_b7 == 0) {
                ...
                if (record->num_stop_meshes_40 == 2) {
                    ...
                    record->pStopMeshes[0].m_frame_58 = last;
                    ++prop_number;
                }
            } else {
