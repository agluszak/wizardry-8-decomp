    ...
    float* path_range)
{
    ...
    if (path_name == 0 || *path_name == 0 || paths_21c == 0 || path_count_220 == 0) {
        return 0;
    }
    index = 0;
    entry = paths_21c;
    do {
        if (strcmp(reinterpret_cast<const char*>(path_name),
            ...
            path_range[0] = 1e+08f;
            path_range[2] = -1e+08f;
            outer = *reinterpret_cast<int*>(paths_21c + index * 0x44 + 0x40) * 4;
            outer_table = m_positional_22c;
            ...
            return *reinterpret_cast<unsigned int*>(paths_21c + index * 0x44 + 0x40);
        }
        ++index;
        entry += 0x44;
    } while (index < (unsigned int)path_count_220);
    return 0;
}
